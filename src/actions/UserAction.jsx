import {
  ...
  ADD_POLICY_REQUEST,
  ADD_POLICY_SUCCESS,
  GET_ALL_POLICIES_FAIL,
  GET_ALL_POLICIES_REQUEST,
  GET_ALL_POLICIES_SUCCESS,
  GET_POLICY_BY_PAN_FAIL,
  GET_POLICY_BY_PAN_REQUEST,
  GET_POLICY_BY_PAN_SUCCESS,
  SIGN_IN_FAIL,
  SIGN_IN_REQUEST,
  SIGN_IN_SUCCESS,
  USER_SIGNOUT,
...
export const userSignin = (username, password) => async (dispatch) => {
  ...
  try {
    const { data } = await Axios.post(`${baseAPI.USER_API}/signin`, {
      username,
      password,
    });
...
export const allPolocyDetailsByPan = (_id) => async (dispatch, getState) => {
  ...
  try {
    const { data } = await Axios.get(
      `${baseAPI.DOCUMENT_API}/policyholder/${_id}`,
      {
        headers: {
          Authorization: `Bearer ${userInfo.token}`,
        },
...
export const addPolicy = (policyHolderDao) => async (dispatch, getState) => {
  console.log(policyHolderDao);
  ...
  try {
    const { data } = await Axios.post(
      `http://localhost:8083/api/policy/addpolicy`,
      policyHolderDao,
